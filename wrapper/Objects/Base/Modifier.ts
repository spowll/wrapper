...
export type ModifierFieldHandler = Map<EModifierfunction, ModifierHandlerValue>
...
export class Modifier {
	...
	public static CanBeHealed(option: Unit | Modifier[]): boolean {
		return Array.isArray(option)
			...
			: option.Modifiers.All.every(buff => !this.DebuffHeal.includes(buff.Name))
	}
	...
	protected DeclaredFunction: Nullable<ModifierFieldHandler>
	...
	public Remove(): boolean {
		if (this.Parent === undefined || !this.Parent.Modifiers.All.includes(this)) {
			return false
		}
		if (this.DeclaredFunction !== undefined) {
			this.DeclaredFunction.forEach((handler, key) =>
				this.Parent!.Modifiers.RemoveInternalModifierFunction(key, handler)
			)
		}
		this.Parent.Buffs.remove(this)
		this.Parent.Modifiers.All.remove(this)
		this.IsValid = false
	...
	protected AddModifier(): boolean {
		...
		if (this.DeclaredFunction !== undefined) {
			this.DeclaredFunction.forEach((handler, key) =>
				this.Parent!.Modifiers.AddInternalModifierFunction(key, handler)
			)
		}

		this.Parent.Buffs.push(this)
		this.Parent.Modifiers.All.push(this)
